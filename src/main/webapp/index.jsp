 fsdsdf<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<!DOCTYPE html>
<html>
<head>
    <title>JSP - Hello World</title>
</head>
<body>
<h1><%= "Servlet BASIC" %>
</h1>
<br/>
<a href="hello-servlet">Hello Servlet</a>
<br/>
<a href="time-servlet">현재 시각</a>
<br/>
<a href="korean-servlet">한글 응답</a>
<br/>
<a href="request-info-servlet">요청 정보</a>
<br/>
<a href="memberInfo.html">쿼리 스트링</a>
<br/>
<a href="practice/multiplication.html">실습 1 : 구구단</a>
<br/>
<a href="calc.html">실습 2 : 계산기</a>
<br/>
<a href="state.store">상태정보 유지 기술 : 애플리케이션 단위 (servletContext)</a>
<br/>
<a href="user-visit.html">상태 정보 유지 기술 : 유저 방문 정보 페이지(세션 활용)</a>
<br/>
<a href="cookie">상태 정보 유지 기술 : 쿠키 만들기 (쿠키 활용)</a>
<br/>
<a href="cookie-read">상태 정보 유지 기술 : 쿠키 읽어오기(쿠키 활용)</a>
<br/>
<a href="site.html">상태 정보 유지 기술 : 요청 단위 (Request)</a>
<br/>
<a href="dispatcher1">포워드</a>
<br/>
<a href="practice/join.html">실습 3 : 회원가입</a>
<br/>
<a href="practice/login.html">실습 3 : 로그인</a>
</body>
</body>
<h4><%="Hello!!"%></h4>
</html>